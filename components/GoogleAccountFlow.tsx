...
function Detail({ route }: { route: Exclude<RouteId, "receive-data"> }) {
  ...
  if (route === "download") return <DetailPage eyebrow="내 구글 계정" title="내 데이터를 내려받고 싶어요" intro="구글 테이크아웃에서 사진, 이메일, 파일 등의 사본을 만들어 보관해요." sections={[
    ["내려받을 수 있는 데이터", ["지메일 이메일", "구글 포토의 사진과 동영상", "구글 드라이브의 문서와 파일", "구글 캘린더, 유튜브, 계정 활동 기록 등"]],
  ...
  if (route === "delete-mine") return <DetailPage eyebrow="내 구글 계정" title="내 구글 계정을 삭제하고 싶어요" intro="더 이상 사용하지 않을 구글 계정과 계정에 연결된 데이터를 삭제해요." warning="삭제하기 전에 필요한 사진, 이메일, 문서를 내려받았는지 확인해 주세요." sections={[
    ...
    ["꼭 알아둘 점", ["지메일뿐 아니라 연결된 여러 구글 서비스와 데이터에 접근할 수 없게 돼요.", "계정으로 로그인하던 외부 서비스에 접근하지 못할 수 있어요.", "최근 삭제한 계정은 복구할 수도 있지만 시간이 지나면 복구하지 못할 수 있어요."]],
...
function DetailPage({ eyebrow, title, intro, sections, warning, note, aside, primary, secondary }: { eyebrow: string; title: string; intro: string; sections: Array<[string, string[]]>; warning?: string; note?: string; aside?: { title: string; text: string; label: string; href: string }; primary?: ActionLink; secondary?: ActionLink }) {
  ...
  return <main className={`${styles.shell} ${styles.compact} ${styles.detail}`}><p className={styles.eyebrow}>{eyebrow}</p><h1>{title}</h1><p className={styles.lead}>{intro}</p>{warning && <div className={styles.warning}><strong>먼저 확인해 주세요</strong><p>{warning}</p></div>}<div className={styles.sections}>{sections.map(([heading, items]) => <section key={heading}><h2>{heading}</h2><ul>{items.map(item => <li key={item}>{item}</li>)}</ul></section>)}</div>{hasRequiredDocuments && <Link className={styles.commonDocumentsLink} href="/prepare#documents">준비 페이지에서 발급 방법 보기 →</Link>}{note && <p className={styles.note}>{note}</p>}{aside && <aside className={styles.aside}><h2>{aside.title}</h2><p>{aside.text}</p><a href={aside.href} target="_blank" rel="noopener noreferrer">{aside.label}</a></aside>}{primary && <OfficialSiteNotice documents={hasRequiredDocuments}/>}<div className={styles.actions}>{secondary && <a className={styles.secondary} href={secondary.href} target="_blank" rel="noopener noreferrer">{secondary.label}</a>}{primary && <a className={styles.primary} href={primary.href} target="_blank" rel="noopener noreferrer">{primary.label}</a>}</div><GuideCompletion company="구글" task={title} href="/company/google" /></main>;
}

function OfficialSiteNotice({documents}:{documents:boolean}){return <section className={styles.officialHandoff}><h2>공식 사이트로 이동하면</h2><ul><li>{documents?"신청자 정보와 증빙서류 제출 화면이 열릴 수 있어요.":"구글 계정 로그인이 필요할 수 있어요."}</li><li>구글 정책에 따라 추가 확인 단계가 표시될 수 있어요.</li><li>이 사이트는 계정 비밀번호나 증빙서류를 받거나 저장하지 않아요.</li></ul></section>}
