      ...
      {audience && (
        <nav className={styles.contextNavigation} aria-label="구글 도움말 내 이동">
          <ol className={styles.breadcrumb}>
            <li>서비스</li>
            <li>구글</li>
            <li>{audience === "mine" ? "내 구글 계정" : "고인의 구글 계정"}</li>
          </ol>
...
function Detail({ route }: { route: Exclude<RouteId, "receive-data"> }) {
  ...
  ]} note="구글에서는 이 기능을 ‘휴면 계정 관리자’라고 부릅니다." primary={{ label: "구글에서 사후 계획 설정하기", href: "https://myaccount.google.com/inactive" }} />;

  if (route === "download") return <DetailPage eyebrow="내 구글 계정" title="내 데이터를 내려받고 싶어요" intro="Google 테이크아웃에서 사진, 이메일, 파일 등의 사본을 만들어 보관해요." sections={[
    ["내려받을 수 있는 데이터", ["Gmail 이메일", "Google 포토의 사진과 동영상", "Google 드라이브의 문서와 파일", "캘린더, YouTube, 계정 활동 기록 등"]],
  ...
  ]} primary={{ label: "Google 테이크아웃에서 데이터 내려받기", href: "https://takeout.google.com/" }} />;
  ...
  if (route === "delete-mine") return <DetailPage eyebrow="내 구글 계정" title="내 구글 계정을 삭제하고 싶어요" intro="더 이상 사용하지 않을 구글 계정과 계정에 연결된 데이터를 삭제해요." warning="삭제하기 전에 필요한 사진, 이메일, 문서를 내려받았는지 확인해 주세요." sections={[
    ["삭제 전에 해야 할 일", ["Google 테이크아웃에서 필요한 데이터를 내려받아요.", "이 이메일로 가입한 외부 서비스의 주소를 변경해요.", "구독과 정기 결제를 확인해요.", "필요한 데이터가 안전하게 저장되었는지 확인해요."]],
  ...
  ]} secondary={{ label: "내 데이터 먼저 내려받기", href: "https://takeout.google.com/" }} primary={{ label: "구글 계정 삭제하기", href: "https://support.google.com/accounts/answer/32046?hl=ko" }} />;
