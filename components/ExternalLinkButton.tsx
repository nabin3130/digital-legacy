...
export default function ExternalLinkButton({ href, label, targetName }: { href: string | null; label: string; targetName: string }) {
  ...
  return <a className={styles.linkButton} href={href} target="_blank" rel="noopener noreferrer" aria-label={`${targetName} ${label} (새 창)`}>{label}<span aria-hidden="true">↗</span></a>;
}
