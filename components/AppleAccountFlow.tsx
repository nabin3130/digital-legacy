...
function Detail({ route }: { route: RouteId }) {
  ...
  if (route === "request-access") return <DetailPage eyebrow="고인의 애플 계정" title="유산 연락처로 계정 접근을 요청하고 싶어요" intro="고인이 남긴 접근 키와 사망 증명서를 준비한 뒤 애플에 계정 접근을 요청해요." sections={[["필요한 서류", ["유산 연락처 접근 키", "사망 증명서"]], ["진행 방법", ["Digital Legacy 웹사이트에서 본인 확인을 해요.", "접근 키를 입력하고 사망 증명서를 제출해요.", "애플의 검토와 승인 결과를 기다려요."]]]} primary={{ label: "계정 접근 요청하기", href: DIGITAL_LEGACY }} />;
  ...
  return <DetailPage eyebrow="고인의 애플 계정" title="접근 키가 없을 때 절차를 확인하고 싶어요" intro="유산 연락처 접근 키가 없다면 법원 명령 또는 지역에서 인정하는 법적 서류를 준비해 애플 지원에 접근을 요청해야 할 수 있어요." sections={[["필요할 수 있는 서류", ["고인이 해당 애플 계정의 사용자였다는 확인", "요청자가 고인의 법적 대리인 또는 상속인임을 확인하는 서류", "계정 정보 공개를 명하는 법원 명령 또는 지역별 대체 법적 서류"]], ["꼭 알아둘 점", ["요구 서류와 처리 가능 여부는 국가와 지역에 따라 달라요.", "애플 지원의 안내에 따라 추가 자료를 제출해야 할 수 있어요."]]]} primary={{ label: "법적 요청 안내 확인하기", href: GUIDE }} />;
}

type ActionLink = { label: string; href: string };
function DetailPage({ eyebrow, title, intro, sections, warning, primary }: { eyebrow: string; title: string; intro: string; sections: Array<[string, string[]]>; warning?: string; primary: ActionLink }) {
  return <main className={`${styles.shell} ${styles.compact} ${styles.detail}`}><p className={styles.eyebrow}>{eyebrow}</p><h1>{title}</h1><p className={styles.lead}>{intro}</p>{warning && <div className={styles.warning}><strong>먼저 확인해 주세요</strong><p>{warning}</p></div>}<div className={styles.sections}>{sections.map(([heading, items]) => <section key={heading}><h2>{heading}</h2><ul>{items.map(item => <li key={item}>{item}</li>)}</ul></section>)}</div><div className={styles.actions}><a className={styles.primary} href={primary.href} target="_blank" rel="noopener noreferrer">{primary.label}</a></div></main>;
}
