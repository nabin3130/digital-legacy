...
type RouteId = "add-contact" | "share-key" | "delete-mine" | "request-access" | "delete-account" | "no-key";
...
function Detail({ route }: { route: RouteId }) {
  ...
  if (route === "request-access") return <DetailPage eyebrow="고인의 애플 계정" title="유산 관리자 접근 키가 있어요" intro="고인이 남긴 접근 키와 사망 증명서를 준비한 뒤 Apple에 계정 접근을 요청해요." sections={[["요청하기 전에 확인할 내용", ["본인이 고인의 유산 관리자로 지정되어 있어야 해요.", "고인이 전달한 접근 키와 사망 증명서가 필요해요.", "제출한 정보는 Apple의 검토를 거쳐요.", "유산 관리자가 여러 명이면 각자 요청할 수 있고, 한 명이 계정 삭제를 요청하면 다른 관리자의 접근에도 영향을 줄 수 있어요."]], ["요청이 승인되면", ["고인의 기존 Apple 계정으로 직접 로그인하는 방식이 아니에요.", "Apple이 제공하는 별도의 유산 관리자용 계정으로 승인된 데이터에 접근해요.", "최초 승인일부터 3년 동안 접근할 수 있으며 이후 해당 계정은 삭제돼요.", "승인 기간 안에 필요한 데이터를 확인하고 내려받아 별도로 보관해요."]], ["접근할 수 있는 데이터", ["iCloud 사진, 메일, 연락처, 캘린더 및 미리 알림", "메모, iCloud에 저장된 메시지, 통화 기록 및 음성 메모", "iCloud Drive의 파일과 iCloud 백업", "건강 데이터와 Safari 책갈피 및 읽기 목록", "실제 제공 범위는 고인이 저장한 데이터와 서비스 설정에 따라 달라져요."]], ["접근할 수 없는 데이터", ["Apple 계정으로 구입한 영화, 음악, 책 및 구독 항목", "앱 내 구입 항목", "Apple Pay에 등록된 결제 정보", "키체인에 저장된 암호와 패스키"]], ["진행 방법", ["Digital Legacy 웹사이트에서 본인 확인을 해요.", "접근 키를 입력하고 사망 증명서를 제출해요.", "Apple의 검토와 승인 결과를 기다려요.", "승인 후 유산 관리자용 계정으로 데이터를 확인하고 내려받아요."]]]} primary={{ label: "계정 접근 요청하기", href: DIGITAL_LEGACY }} />;
  if (route === "delete-account") return <DetailPage eyebrow="고인의 애플 계정" title="고인의 애플 계정 삭제를 요청하고 싶어요" intro="고인의 데이터에 접근할 필요가 없고 계정만 영구적으로 삭제하려는 경우 이용하는 절차예요." warning="필요한 데이터가 있다면 계정을 삭제하기 전에 먼저 접근 또는 내려받기 절차를 진행해 주세요." sections={[["필요한 정보와 서류", ["요청자 본인의 Apple 계정", "고인의 Apple 계정 정보", "고인의 사망 증명서", "요청자와 고인의 관계 또는 법적 권한을 증명하는 서류", "국가·지역에 따라 요구되는 법원 명령서나 대체 서류"]], ["Apple 계정이 왜 필요한가요?", ["요청자 본인의 신원을 확인하고 삭제 요청을 제출하려면 본인 Apple 계정으로 로그인해야 해요.", "이 계정은 고인의 계정에 접근하기 위한 계정이 아니에요.", "Apple 계정이 없다면 삭제 요청을 시작하기 전에 본인 명의의 계정을 만들어야 해요.", "새로 만든 계정으로 고인의 데이터에 접근할 수는 없어요."]], ["꼭 알아둘 점", ["삭제가 완료되면 계정과 데이터는 복구할 수 없어요.", "Apple이 요청자의 권한과 제출 서류를 검토해요."]]]} secondary={{ label: "Apple 계정 생성 방법 확인하기", href: "https://support.apple.com/ko-kr/108647" }} primary={{ label: "고인의 계정 삭제 요청하기", href: DIGITAL_LEGACY }} />;
  return <DetailPage eyebrow="고인의 애플 계정" title="접근 키가 없을 때 절차를 확인하고 싶어요" intro="고인이 유산 관리자를 지정하지 않았거나 접근 키를 찾을 수 없다면, 법적 요청 절차를 확인해야 해요." sections={[["필요할 수 있는 정보와 서류", ["고인과의 관계를 증명하는 정보", "요청자가 고인의 법적 대리인 또는 상속인임을 증명하는 서류", "고인의 사망 증명서", "법원 명령서 또는 국가·지역에서 인정되는 대체 법적 서류"]], ["꼭 알아둘 점", ["요구 서류와 처리 가능 여부는 국가와 지역에 따라 달라요.", "Apple의 안내에 따라 추가 자료를 제출해야 할 수 있어요.", "서류를 제출하더라도 요청이 반드시 승인되는 것은 아니에요."]]]} image={{ src: "/images/apple-no-access-key.png", alt: "Apple 디지털 유산 웹사이트에서 접근 키가 없을 때 선택하는 위치" }} primary={{ label: "법적 요청 안내 확인하기", href: DIGITAL_LEGACY }} />;
}
...
function DetailPage({ eyebrow, title, intro, sections, warning, image, primary, secondary }: { eyebrow: string; title: string; intro: string; sections: Array<[string, string[]]>; warning?: string; image?: { src: string; alt: string }; primary: ActionLink; secondary?: ActionLink }) {
  return <main className={`${styles.shell} ${styles.compact} ${styles.detail}`}><p className={styles.eyebrow}>{eyebrow}</p><h1>{title}</h1><p className={styles.lead}>{intro}</p>{warning && <div className={styles.warning}><strong>먼저 확인해 주세요</strong><p>{warning}</p></div>}<div className={styles.sections}>{sections.map(([heading, items]) => <section key={heading}><h2>{heading}</h2><ul>{items.map(item => <li key={item}>{item}</li>)}</ul></section>)}</div>{image && <figure className={styles.guideImage}><img src={image.src} alt={image.alt} /><figcaption>Apple Digital Legacy 페이지에서 ‘접근 키를 가지고 있지 않습니다’를 선택해 해당 지역에 필요한 법적 절차를 확인해요.</figcaption></figure>}<div className={styles.actions}>{secondary && <a className={styles.secondary} href={secondary.href} target="_blank" rel="noopener noreferrer">{secondary.label}</a>}<a className={styles.primary} href={primary.href} target="_blank" rel="noopener noreferrer">{primary.label}</a></div></main>;
}
