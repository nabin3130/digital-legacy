...
function DetailPage({ eyebrow, title, intro, sections, warning, image, primary, secondary }: { eyebrow: string; title: string; intro: string; sections: Array<[string, string[]]>; warning?: string; image?: { src: string; alt: string }; primary: ActionLink; secondary?: ActionLink }) {
  ...
  return <main className={`${styles.shell} ${styles.compact} ${styles.detail}`}><p className={styles.eyebrow}>{eyebrow}</p><h1>{title}</h1><p className={styles.lead}>{intro}</p>{warning && <div className={styles.warning}><strong>먼저 확인해 주세요</strong><p>{warning}</p></div>}<div className={styles.sections}>{sections.map(([heading, items]) => <section key={heading}><h2>{heading}</h2><ul>{items.map(item => <li key={item}>{item}</li>)}</ul></section>)}</div>{hasRequiredDocuments && <Link className={styles.commonDocumentsLink} href="/documents">공통 서류에서 발급 방법 보기 →</Link>}{image && <figure className={styles.guideImage}><img src={image.src} alt={image.alt} /><figcaption>애플 디지털 유산 페이지에서 ‘접근 키를 가지고 있지 않습니다’를 선택해 해당 지역에 필요한 법적 절차를 확인해요.</figcaption></figure>}<div className={styles.actions}>{secondary && <a className={styles.secondary} href={secondary.href} target="_blank" rel="noopener noreferrer">{secondary.label}</a>}<a className={styles.primary} href={primary.href} target="_blank" rel="noopener noreferrer">{primary.label}</a></div></main>;
}
